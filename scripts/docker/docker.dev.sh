...
# Script to run the app in development with hot reload
# Usage: ./docker.dev.sh [up|down|logs|rebuild]
...
case $COMMAND in
  up)
    echo "Starting development services (hot reload enabled)..."
  ...
  rebuild)
    ...
    $DOCKER_COMPOSE -f docker-compose.dev.yml up --build --no-cache
    ;;
  *)
    echo "Usage: $0 {up|down|logs|rebuild}"
    echo ""
